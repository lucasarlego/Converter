...
class CSVtoXLSXConverterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CSV to XLSX Converter")
        self.root.geometry("700x200")  # Ajuste o tamanho da janela conforme necessário

        self.file_path = ""
        self.status_var = tk.StringVar()
        self.conversion_status_var = tk.StringVar()  # Variável para a mensagem de status da conversão

        self.path_frame = tk.Frame(self.root)
        self.path_frame.pack(fill=tk.X, padx=20, pady=(20, 10))

        self.path_label = tk.Label(self.path_frame, text="Diretório:")
        ...
        self.choose_button = tk.Button(self.path_frame, text="Escolher Arquivo", command=self.choose_csv_file)
        self.choose_button.pack(side=tk.LEFT, padx=(10, 0))

        self.convert_button = tk.Button(self.root, text="Converter para XLSX", command=self.convert_csv_to_xlsx)
    ...
    def convert_csv_to_xlsx(self):
        if self.file_path:
            ...
            if output_path:
                ...
                self.conversion_status_var.set("Conversão concluída!")

if __name__ == "__main__":
    app_root = tk.Tk()
    app = CSVtoXLSXConverterApp(app_root)
    app_root.mainloop()
